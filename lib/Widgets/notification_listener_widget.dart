...
// foundation types are available via material.dart when needed
...
class _NotificationListenerWidgetState
  ...
  Widget build(BuildContext context) {
    ...
    if (currentUser != null) {
      ...
      ref.listen<
        AsyncValue<List<Swap>>
      >(receivedOffersProvider(currentUser.uid), (previous, next) {
        next.when(
          data: (swaps) async {
            debugPrint(
              'NotificationListener: Received ${swaps.length} swap(s)',
            );
            if (_lastSwapIds.isEmpty) {
              ...
              debugPrint(
                'NotificationListener: First load - lastActiveTime: $lastActiveTime',
              );
              ...
              if (lastActiveTime != null) {
                ...
                debugPrint(
                  'NotificationListener: Checking ${swaps.length} swaps against lastActiveTime: $lastActiveTime',
                );
                for (final swap in swaps) {
                  debugPrint(
                    'NotificationListener: Swap ${swap.id} - createdAt: ${swap.createdAt}, status: ${swap.status}, isAfter: ${swap.createdAt.isAfter(lastActiveTime)}',
                  );
                }
                final offlineSwaps = swaps
                    .where(
                      (swap) =>
                ...
                debugPrint(
                  'NotificationListener: Found ${offlineSwaps.length} offline swap(s)',
                );

                if (offlineSwaps.isNotEmpty) {
                  debugPrint(
                    'NotificationListener: Found ${offlineSwaps.length} swap(s) that arrived while offline',
                  );
                  ...
                  debugPrint(
                    'NotificationListener: No offline swaps found (all swaps are older than lastActiveTime or not pending)',
                  );
                }
              } else {
                debugPrint(
                  'NotificationListener: No lastActiveTime found (first time user or data cleared)',
                );
              }
            ...
            if (newSwapIds.isNotEmpty) {
              debugPrint(
                'NotificationListener: Found ${newSwapIds.length} new swap(s)',
              );
      ...
      ref.listen<AsyncValue<List<Chat>>>(userChatsProvider(currentUser.uid), (
        previous,
        next,
      ) {
        next.when(
          data: (chats) {
            debugPrint(
              'NotificationListener: Received ${chats.length} chat(s)',
            );
            WidgetsBinding.instance.addPostFrameCallback((_) {
              final currentChatIds = chats.map((c) => c.id).toSet();
              final newChatIds = currentChatIds.difference(_listeningChatIds);
              final removedChatIds = _listeningChatIds.difference(
                currentChatIds,
              );

              debugPrint(
                'NotificationListener: Setting up listeners for ${newChatIds.length} new chat(s)',
              );
  ...
  void _setupMessageListener(String chatId, String currentUserId) {
    debugPrint(
      'NotificationListener: Setting up message listener for chat $chatId',
    );
    // Use ref.listenManual to set up listeners dynamically
    final subscription = ref.listenManual<AsyncValue<List<msg.Message>>>(chatMessagesProvider(chatId), (
      previous,
      next,
    ) {
      next.when(
        data: (messages) async {
          debugPrint(
            'NotificationListener: Received ${messages.length} message(s) in chat $chatId',
          );
          if (!_lastMessageIds.containsKey(chatId)) {
            // First load - check for messages created while user was offline
            final lastActiveTime = await _getLastActiveTime();
            debugPrint(
              'NotificationListener: First load for chat $chatId - lastActiveTime: $lastActiveTime',
            );
            ...
            if (lastActiveTime != null) {
              ...
              debugPrint(
                'NotificationListener: Checking ${messages.length} messages in chat $chatId against lastActiveTime: $lastActiveTime',
              );
              for (final message in messages.take(5)) {
                // Log first 5 messages
                debugPrint(
                  'NotificationListener: Message ${message.id} - timestamp: ${message.timestamp}, senderId: ${message.senderId}, isAfter: ${message.timestamp.isAfter(lastActiveTime)}, isNotMine: ${message.senderId != currentUserId}',
                );
              }
              final offlineMessages = messages
                  .where(
                    (m) =>
              ...
              debugPrint(
                'NotificationListener: Found ${offlineMessages.length} offline message(s)',
              );

              if (offlineMessages.isNotEmpty) {
                debugPrint(
                  'NotificationListener: Found ${offlineMessages.length} message(s) in chat $chatId that arrived while offline',
                );
                // Show notification for the most recent offline message
                final mostRecentMessage = offlineMessages.reduce(
                  (a, b) => a.timestamp.isAfter(b.timestamp) ? a : b,
                );
                _showMessageNotification(mostRecentMessage, chatId);
              } else {
                debugPrint(
                  'NotificationListener: No offline messages found in chat $chatId',
                );
              }
            } else {
              debugPrint(
                'NotificationListener: No lastActiveTime found for chat $chatId',
              );
            }
          ...
          debugPrint(
            'NotificationListener: Comparing messages in chat $chatId - previous: ${previousIds.length}, next: ${nextIds.length}',
          );

          // Find new messages that weren't sent by current user
          final newMessageIds = nextIds.difference(previousIds);
          debugPrint(
            'NotificationListener: Found ${newMessageIds.length} new message ID(s) in chat $chatId',
          );

          if (newMessageIds.isNotEmpty) {
            debugPrint(
              'NotificationListener: New message IDs: ${newMessageIds.take(3).join(", ")}',
            );
            final newMessages = messages
                .where(
                  (m) =>
                      newMessageIds.contains(m.id) &&
                      m.senderId != currentUserId,
                )
                .toList();

            debugPrint(
              'NotificationListener: Filtered to ${newMessages.length} new message(s) not sent by current user',
            );

            if (newMessages.isNotEmpty) {
              debugPrint(
                'NotificationListener: Found ${newMessages.length} new message(s) in chat $chatId',
              );
              final newMessage = newMessages.first;
              _showMessageNotification(newMessage, chatId);
            } else {
              debugPrint(
                'NotificationListener: All new messages were sent by current user, skipping notification',
              );
            }
          } else {
            debugPrint(
              'NotificationListener: No new message IDs found (messages might have been removed or reordered)',
            );
          }
        ...
        loading: () {
          debugPrint(
            'NotificationListener: Messages for chat $chatId are loading...',
          );
        },
        error: (error, stackTrace) {
          debugPrint(
            'NotificationListener: Error loading messages for chat $chatId: $error',
          );
          debugPrint('NotificationListener: Stack trace: $stackTrace');
        },
      );
  ...
  Future<void> _showSwapNotification(Swap swap) async {
    try {
      debugPrint(
        'NotificationListener: Attempting to show swap notification for swap ${swap.id}',
      );
      // Get book title
      ...
      await bookAsync.when(
        data: (book) async {
          ...
          debugPrint(
            'NotificationListener: Showing swap notification - $requesterName wants "${book.title}"',
          );
        ...
        loading: () async {
          ...
          await bookAsyncRetry.when(
            data: (book) async {
              final requesterName = swap.requesterEmail.split('@')[0];
              debugPrint(
                'NotificationListener: Showing swap notification (retry) - $requesterName wants "${book.title}"',
              );
              await _notificationService.showSwapRequestNotification(
                requesterName: requesterName,
                bookTitle: book.title,
            ...
            loading: () {
              debugPrint(
                'NotificationListener: Book still loading, using fallback',
              );
        ...
        error: (error, stack) async {
          debugPrint(
            'NotificationListener: Error loading book for notification: $error',
          );
          _showSwapNotificationFallback(swap);
        },
      );
  ...
  Future<void> _showSwapNotificationFallback(Swap swap) async {
    try {
      final requesterName = swap.requesterEmail.split('@')[0];
      debugPrint(
        'NotificationListener: Showing swap notification (fallback) - $requesterName wants a book',
      );
      await _notificationService.showSwapRequestNotification(
        requesterName: requesterName,
        bookTitle: 'a book',
  ...
  Future<void> _showMessageNotification(
    ...
    try {
      debugPrint(
        'NotificationListener: Attempting to show message notification for chat $chatId',
      );
      ...
      await chatAsync.when(
        data: (chat) async {
          ...
          debugPrint(
            'NotificationListener: Showing message notification - $senderName: $messagePreview (photo: ${senderPhotoURL != null ? "yes" : "no"})',
          );
        ...
        loading: () async {
          ...
          await chatAsyncRetry.when(
            data: (chat) async {
              ...
              debugPrint(
                'NotificationListener: Showing message notification (retry) - $senderName: $messagePreview (photo: ${senderPhotoURL != null ? "yes" : "no"})',
              );
              await _notificationService.showMessageNotification(
                senderName: senderName,
                messageText: messagePreview,
            ...
            loading: () {
              debugPrint(
                'NotificationListener: Chat still loading, using fallback',
              );
        ...
        error: (error, stack) async {
          debugPrint(
            'NotificationListener: Error loading chat for notification: $error',
          );
          _showMessageNotificationFallback(message, chatId);
        },
      );
  ...
  Future<void> _showMessageNotificationFallback(
    msg.Message message,
    String chatId,
  ) async {
    try {
      ...
      debugPrint(
        'NotificationListener: Showing message notification (fallback) - $senderName: $messagePreview',
      );
      await _notificationService.showMessageNotification(
        senderName: senderName,
        messageText: messagePreview,
