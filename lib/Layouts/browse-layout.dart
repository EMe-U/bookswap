...
class BrowseLayout extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // Reserve space at the bottom for the app's bottom navigation bar so lists
    // don't get covered and produce bottom overflow errors (especially on web
    // where browser insets may add extra UI). Use kBottomNavigationBarHeight
    // plus the device bottom inset.
    final bottomInset = MediaQuery.of(context).padding.bottom;
    final navHeight = kBottomNavigationBarHeight + bottomInset;
        ...
        return allBooksAsync.when(
          data: (books) {
            ...
            return ListView.builder(
              padding: EdgeInsets.only(bottom: navHeight + 16),
...
class _BookCard extends ConsumerWidget {
  ...
  const _BookCard({required this.book, this.currentUserId});
  ...
  Future<void> _requestSwap(BuildContext context, WidgetRef ref) async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
          'Are you sure you want to request a swap for "${book.title}"?',
        ),
        actions: [
          TextButton(
    ...
    try {
      ...
      await swapService.createSwapOffer(bookId: book.id, book: book);
