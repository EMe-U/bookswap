...
class NotificationService {
  ...
  Future<void> initialize() async {
    ...
    try {
      ...
      if (initialized == false) {
        debugPrint(
          'Notification service: Platform initialization returned false',
        );
        return;
      }
      ...
      debugPrint(
        'Note: This may require a full app rebuild (not just hot reload)',
      );
  ...
  Future<void> showSwapRequestNotification({
    ...
    debugPrint(
      'NotificationService: Showing swap notification (ID: $notificationId)',
    );
  ...
  Future<void> showMessageNotification({
    ...
    if (senderPhotoURL != null && senderPhotoURL.isNotEmpty) {
      try {
        ...
        if (imageBytes != null) {
          ...
          debugPrint(
            'NotificationService: Downloaded avatar image bytes (${imageBytes.length} bytes)',
          );
        }
        ...
        if (largeIconPath != null) {
          debugPrint(
            'NotificationService: Downloaded avatar image to file: $largeIconPath',
          );
        }
      } catch (e) {
    ...
    final iosDetails = DarwinNotificationDetails(
      ...
      attachments: largeIconPath != null
          ? [DarwinNotificationAttachment(largeIconPath, identifier: 'avatar')]
          : null,
    );
    ...
    final notificationId = DateTime.now().millisecondsSinceEpoch % 100000;
    debugPrint(
      'NotificationService: Showing message notification (ID: $notificationId) - $senderName',
    );
    await _notifications.show(
      notificationId,
      senderName,
  ...
  Future<Uint8List?> _downloadImageBytes(String imageUrl) async {
    try {
      debugPrint(
        'NotificationService: Downloading avatar bytes from $imageUrl',
      );
      ...
      if (response.statusCode == 200) {
        ...
        debugPrint(
          'NotificationService: Failed to download avatar - status code: ${response.statusCode}',
        );
      }
    } catch (e) {
  ...
  Future<String?> _downloadImageForNotification(String imageUrl) async {
    try {
      ...
      if (imageBytes != null) {
        ...
        final filePath =
            '${directory.path}/notification_avatar_${DateTime.now().millisecondsSinceEpoch}.jpg';
