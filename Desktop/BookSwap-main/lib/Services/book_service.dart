...
class BookService {
  ...
  Future<Book> createBook({
    ...
    required dynamic coverImageFile, // File on mobile, Uint8List on web
  }) async {
    ...
    try {
      ...
      final String fileName =
          'book_covers/${user.uid}/${DateTime.now().millisecondsSinceEpoch}.jpg';
      ...
      debugPrint(
        ' Image uploaded successfully. Bytes: ${snapshot.bytesTransferred}',
      );
      ...
      if (e.code == 'object-not-found' || e.code == '-13010') {
        errorMessage =
            'Firebase Storage is not enabled or not configured. '
  ...
  Future<Book> updateBook({
    ...
    dynamic coverImageFile, // Optional: File or Uint8List
  }) async {
    final User? user = _auth.currentUser;
    if (user == null) {
