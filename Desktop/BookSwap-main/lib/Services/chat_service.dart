...
// Notification service import removed because it's not used in this file

/// Service class for managing chat conversations and messages
class ChatService {
  final FirebaseFirestore _firestore = FirebaseFirestore.instance;
  final FirebaseAuth _auth = FirebaseAuth.instance;
  // Notification service not used directly here; keep available if needed in future
    ...
    try {
      ...
      for (var doc in existingChats.docs) {
        ...
        if (chat.participants.contains(userId1) &&
          ...
          final needsUpdate =
              (user.uid == userId1 &&
                  (!chat.participantNames.containsKey(userId1) ||
                      chat.participantNames[userId1] != name1)) ||
              (user.uid == userId2 &&
                  (!chat.participantNames.containsKey(userId2) ||
                      chat.participantNames[userId2] != name2));

          if (needsUpdate) {
            final updatedEmails = Map<String, String>.from(
              chat.participantEmails,
            );
            final updatedNames = Map<String, String>.from(
              chat.participantNames,
            );
            final updatedPhotoURLs = Map<String, String>.from(
  ...
  Future<void> _enrichChatWithSwapInfo(Chat chat) async {
    ...
    try {
      ...
      if (requesterId != null &&
        ...
        if (!updatedEmails.containsKey(requesterId) ||
      ...
      if (ownerId != null && ownerEmail != null && ownerEmail.isNotEmpty) {
        if (!updatedEmails.containsKey(ownerId) ||
